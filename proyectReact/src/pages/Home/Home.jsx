...
//     };
    ...
    // var client_id = 'a03ac48995bb4455ad29c0c7ae199ddf';
    // var redirect_uri = 'https://react-movie-series-pwa-woad.vercel.app/';
    ...
    const clientId = 'a03ac48995bb4455ad29c0c7ae199ddf';
    const redirectUri = 'https://react-movie-series-pwa-woad.vercel.app/';
    ...
    function Home() {
      ...
      useEffect(() => {
        if (token) {
          ...
          // Buscar canción de prueba
          fetch('https://api.spotify.com/v1/search?q=blinding%20lights&type=track&limit=1', {
            headers: { Authorization: 'Bearer ' + token },
          })
            .then(res => res.json())
            .then(data => {
      ...
      return (
        <div style={{ padding: 20 }}>
          {!token && <button onClick={handleLogin}>Login con Spotify</button>}
          ...
          {track && (
            <div style={{ marginTop: 30 }}>
              ...
              {track.album.images[0] && <img src={track.album.images[0].url} alt="cover" width="200" />}
              {track.preview_url ? (
                <audio controls src={track.preview_url} />
              ) : (
                <p>Este track no tiene preview disponible 😢</p>
              )}
            </div>
          )}
        </div>
